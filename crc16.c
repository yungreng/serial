...
static const unsigned char tab_CRCLo[] = {
...
int Crc_calculateCRC( unsigned char *pData, int dataLen)
{
    ...
    while( dataLen-- ){
        iIndex = CRCLo ^ *( pData++ );
        CRCLo = ( unsigned char )( CRCHi ^ tab_CRCHi[iIndex] );
        CRCHi = tab_CRCLo[iIndex];
    }
...
BOOL Crc_checkCRC(Crc *crc, unsigned char *pData, int dataLen)
{
    ...
    int crc_calc;
    int i;
    int payloadLen = dataLen - crc->crc_bytes;
    ...
    crc_calc = crc->calculateCRC(pData,payloadLen);
